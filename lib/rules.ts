export const TOURNAMENT_RULES_MARKDOWN = `
# RageX — Official Rules & Regulations

**Games:** Valorant (PC, Team) | Tekken 8 (PS5, Solo)
**Format:** LAN (on-site, in-person) Tournament
**Last Updated:** 22/06/2026

By registering, every player agrees and team coach to the rules below in full. Tournament admins reserve the right to make final decisions on any situation not explicitly covered here.

---

## 1. Eligibility & Registration

1.1 Registration is open to all players who complete the official Google Form and agree to these rules.
1.2 Players must provide accurate, real information (name, contact number, email, in-game ID/IGN) at registration. Fake or incomplete entries may be disqualified without refund.
1.3 Minimum age: **[14+ years]**,The organizers assume no liability for any confusion, misunderstandings, complaints, or allegations arising from parents/guardians.
1.4 Each player may register for **only one team** in Valorant. Multi-registration (smurfing across teams) is not allowed.
1.5 Tekken 8 is solo entry only — one player, one slot.
1.6 Registration is only confirmed once **payment is received** (see Section 2) and confirmation is sent by the organizers.

---

## 2. Entry Fee & Prize Pool

2.1 **Entry Fee:**
- Valorant 5v5 (per team): ₹1495
- Valorant 2v2 Skirmish (per team): ₹398
- Tekken 8 (per player): ₹229

2.2 Entry fees are **non-refundable** once registration is confirmed, except in cases where the organizers cancel the event entirely.
2.3 **Prize Pool:** Total prize pool of ₹15000, distributed as:
- Valorant: Winner can win upto 5k followed by runner up
- Tekken 8: Winner can win upto 2.5k followed by runner up

2.4 Prize money will be transferred **same day** after the tournament concludes, Cash Prize will be provided by the admins.
2.5 Taxes, if applicable on prize winnings, are the responsibility of the recipient as per local law.
2.6 Organizers reserve the right to adjust the prize pool if the minimum number of registrations is not met, with prior notice to participants.

---

## 3. Check-In & Attendance

3.1 All players must physically check in at the venue at least **5 minutes** before their scheduled match time, with a valid government-issued photo ID matching their registration details.
3.2 Teams/players who fail to check in by the deadline may be marked as a **no-show** and disqualified, forfeiting the match and entry fee.
3.3 Late entries after this may not be permitted entry on the match day.

---

## 4. Equipment & Setup

4.1 **Valorant (PC):** Tournament-provided PCs/peripherals will be used unless otherwise stated. 
4.2 **Tekken 8 (PS5):** Tournament-provided PS5 consoles and standard controllers will be used. 
4.3 Players are responsible for any personal equipment brought to the venue. Organizers are not liable for loss, theft, or damage to personal property.
4.4 No external software, scripts, macros, cheat engines, overlays, or unauthorized hardware modifications are allowed on tournament systems.

---

## 5. Match Rules

### 5.1 Valorant
- Format: Every round is an elimination round.
- Map pool and map selection/ban process: Will be mentioned on our page before tournament
- Agent select, side selection, and overtime rules follow standard official Valorant competitive rules unless stated otherwise.
- Pause requests are limited to **[1 pause]** per team per map, for technical issues only.

### 5.2 Tekken 8
- Format: Knockout Tournament.
- Character select: Any Character
- Stage selection: Random
- Standard Tekken 8 ranked match settings apply (no custom input lag, no slow-motion, no handicaps) unless stated.

5.3 Match start times are firm. A grace period of **[5 minutes]** is given for technical issues only; beyond that, the match may be forfeited.
5.4 All matches will be played exclusively on tournament-provided hardware/network unless an admin grants an exception.
5.5 Results must be reported to the admin desk immediately after each match. Disputes must be raised on the spot — results will not be reviewed after the next match has started.

---

## 6. Code of Conduct

6.1 **Zero tolerance** for cheating, hacking, exploiting bugs/glitches, match-fixing, or collusion between teams/players. Violators will be **immediately disqualified** with no refund and may be banned from future events.
6.2 No toxic behavior: harassment, hate speech, discrimination, threats, or abusive language toward other players, staff, or spectators (verbal or written) will not be tolerated.
6.3 No unsportsmanlike conduct, including intentional game-throwing, stalling, or disrespecting opponents.
6.4 Players must follow all instructions given by tournament admins/referees during the event. Admin decisions on rule violations are final.
6.5 Any form of betting, bribery, or attempts to influence match outcomes outside of fair play is strictly prohibited.
6.6 Venue rules apply at all times — no smoking/alcohol/illegal substances on premises, and respect for venue property is mandatory.
6.7 Damage caused by a participant may result in liability for repair costs.

---

## 7. Disqualification & Penalties

A player or team may be disqualified, forfeited, or penalized for:
- Failure to check in or show up on time
- Cheating, hacking, or use of unauthorized software/hardware
- Verified collusion or match-fixing
- Abusive, threatening, or discriminatory behavior
- Violating equipment rules
- Repeated rule violations after a warning
Involvement in any malpractices results in Compensation of ₹1000.

Disqualification results in forfeiture of entry fee, prize eligibility, and removal from the venue if necessary. Organizer decisions are final and binding.

---

## 8. Streaming, Media & Privacy

8.1 The tournament may be live-streamed, recorded, and photographed. By participating, players consent to their gameplay, name/IGN, and likeness being used in event broadcasts, social media, and promotional material.
8.2 Personal information collected at registration (name, email, phone number, payment details) will only be used for tournament management and will not be shared with third parties, in accordance with applicable data protection norms.

---

## 9. Liability & Health/Safety

9.1 Participants attend and play at their own risk. Organizers are not liable for any injury, health issue, loss, or damage occurring at the venue.
9.2 Participants confirm they are physically fit to participate in an extended-duration LAN event.
9.3 Organizers reserve the right to refuse entry or remove any participant who poses a safety or health risk to others.

---

## 10. Force Majeure & Changes

10.1 Organizers reserve the right to modify the schedule, format, venue, or rules due to circumstances beyond their control (technical failure, venue issues, weather, etc.), with participants notified as soon as possible.
10.2 In case of event cancellation due to force majeure, refund policy will be: full refund.

---

## 11. Agreement

By submitting the registration form, I confirm that:
- [ ] I have read and agree to all rules and regulations stated above.
- [ ] All information provided by me is accurate and true.
- [ ] I understand that entry fees are non-refundable except as stated in Section 10.
- [ ] I agree to follow the Code of Conduct (Section 6) at all times during the event.
- [ ] I consent to being recorded/photographed for streaming and promotional use (Section 8).

**For queries, contact:** [ragesfankx@gmail.com/Instagram-@rageee__x]

---
`;
